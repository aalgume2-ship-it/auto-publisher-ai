...
generator client {
  ...
  previewFeatures = ["postgresqlExtensions", "driverAdapters"] // clientExtensions is GA in 5.x — no flag needed
}
...
/// Durable binary payload backing every Asset row.
///
/// Render free-tier instance disks are EPHEMERAL (/tmp is wiped on
/// spin-down/restart), so media bytes cannot live on disk alone: AssetStore
/// uses the disk strictly as a hot cache and this table as the durable tier
/// (put writes both; read rehydrates the cache from here on a miss).
///
/// It MUST be a first-class Prisma model — an earlier raw-SQL
/// `CREATE TABLE IF NOT EXISTS` (ensure-vector.mjs) ran BEFORE
/// `prisma db push` at boot, and db push DROPS any table absent from this
/// schema (verified live: boot → table silently gone → generation 42P01).
model AssetBlob {
  storageKey String   @id @map("storage_key") @db.Text
  data       Bytes
  createdAt  DateTime @default(now()) @map("created_at")
...
model OutboxEvent {
  ...
  type          String                                  // aca.<domain>.<verb-path>
  version       Int       @default(1)
  payload       Json                                    // FULL EventEnvelope v1.1 JSON (payload nested) — replay/DLQ truth format (ADR-024)
...
model ConsumerCursor {
  ...
  lastCommittedId  String   @map("last_committed_id")  // JSON map { "<slot>": "<redis entry id>" } — last XACKed per shard (ADR-024)
  updatedAt        DateTime @updatedAt @map("updated_at")
...
model ImageGeneration {
  ...
  count          Int            @default(1)
  status         ImageGenStatus @default(QUEUED)
...
model DubbingJob {
  ...
  orgId             String        @map("organization_id") @db.Uuid
  videoId           String        @map("video_id") @db.Uuid
  sourceLanguage    String        @default("ar") @map("source_language")
  targetLanguage    String        @map("target_language")
  voiceId           String?       @map("voice_id") @db.Uuid
  status            DubbingStatus @default(QUEUED)
  outputRenditionId String?       @map("output_rendition_id") @db.Uuid
  failureReason     String?       @map("failure_reason")
  createdById       String?       @map("created_by_id") @db.Uuid
  finishedAt        DateTime?     @map("finished_at")
  createdAt         DateTime      @default(now()) @map("created_at")
...
model Campaign {
  ...
  orgId             String         @map("organization_id") @db.Uuid
  name              String
  platforms         String[]       // registry ids: youtube | tiktok | instagram
  cadence           String         @default("daily") // daily | weekly | custom
  timeOfDay         String         @default("18:00") @map("time_of_day") // HH:mm
  timezone          String         @default("UTC")
  contentMode       String         @default("auto") @map("content_mode") // auto | manual
  referenceImageIds String[]       @default([]) @map("reference_image_ids")
  config            Json           @default("{}") // { captions: 'ai', hashtags: 'ai', voice: ..., imageStyle: ..., videoStyle: ... }
...
model CampaignPost {
  ...
  publishedTaskId String?          @map("published_task_id") @db.Uuid
  failureReason String?            @map("failure_reason")
  publishedAt   DateTime?          @map("published_at")
  createdAt     DateTime           @default(now()) @map("created_at")
  updatedAt     DateTime           @updatedAt @map("updated_at")
