...
export default function Video(){
  return <PlatformShell>
    ...
    <div className="cp-tabs" aria-label="Video generation modes">{modes.map(([label,prompt])=><Link key={label} href={`/video?prompt=${encodeURIComponent(prompt)}&preset=${encodeURIComponent(label)}`}>{label}</Link>)}</div>
    <Suspense fallback={<div className="cp-composer-wrap" style={{minHeight:420}}/>}><GenerationComposer/></Suspense>
