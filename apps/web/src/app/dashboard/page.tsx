...
function DashboardInner() {
  ...
  // Guest mode: the library is local-only (saved in localStorage).
  const [videos, setVideos] = useState<VideoDto[] | null>([]);
  ...
  useEffect(() => {
    ...
    if (session.mode === 'guest') {
      try {
        ...
        const parsed = raw ? (JSON.parse(raw) as VideoDto[]) : [];
        setVideos(parsed);
      } catch {
        setVideos([]);
      }
    ...
    // Real auth path (kept for parity; unreachable while guest is the default).
    let cancelled = false;
    (async () => {
  ...
  return (
    <div dir="ltr" className="studio-root">
      ...
      <main className="shell" style={{ paddingTop: 24 }}>
        ...
        <span className="pill-note" style={{ marginBottom: 16 }}>
          <Clapperboard size={13} /> Local library (preview)
        </span>
              ...
              return (
                ...
                >
                  <div className="row between">
                    <span className={`chip ${ready ? 'on' : ''}`} style={{ pointerEvents: 'none' }}>
                      {STATUS_LABEL[v.status] ?? 'Processing'}
                    </span>
                  </div>
                  ...
                  <div className="row" style={{ marginTop: 'auto' }}>
                    {ready ? (
                      <Link
                        ...
                        style={{ flex: 1 }}
                        href={`/result?mode=guest&videoId=${v.id}&orgId=${session?.orgId ?? 'guest'}`}
                      >
                        Open
                      </Link>
                    ) : (
