...
export class ChannelsService {
  ...
  private googleConfig(): { clientId: string; clientSecret: string; redirectUri: string } {
    ...
    if (!googleClientId || !googleClientSecret || !redirectUri) {
      throw new ApiError('PLATFORM_ERROR', 'Service Unavailable', {
        status: 503,
        detail:
          'YouTube OAuth client not configured: set GOOGLE_CLIENT_ID + GOOGLE_CLIENT_SECRET (+ PUBLIC_API_URL) on the API service, with redirect URI registered as ' +
          (redirectUri ?? 'https://<api-host>/v1/channels/oauth/youtube/callback') +
          ' in Google Cloud Console → APIs & Services → Credentials.',
      });
    }
    return { clientId: googleClientId, clientSecret: googleClientSecret, redirectUri };
  }
  ...
  async completeYoutubeCallback(code: string | undefined, state: string | undefined): Promise<{ redirectTo: string }> {
    ...
    const { clientId, clientSecret, redirectUri } = this.googleConfig();
