...
export const GenerateVideoBody = z
  .object({
    ...
    /** Optional per-campaign engine. Missing keeps the existing automatic order. */
    videoProvider: z.enum(['local', 'auto', 'pictory', 'd-id', 'runway', 'luma', 'fal-kling']).default('auto'),
    language: z.string().min(2).max(8).default('ar'),
...
export const GenerateVideoBodyDoc = {
  ...
  properties: {
    ...
    videoProvider: { type: 'string', enum: ['local', 'auto', 'pictory', 'd-id', 'runway', 'luma', 'fal-kling'], example: 'local' },
    language: { type: 'string', example: 'ar' },
