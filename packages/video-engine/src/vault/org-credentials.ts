/**
 * OrgCredentialsService — per-organization secret vault built on the
 * `provider_credentials` table (AES-256-GCM envelope via @aca/auth).
 * Namespaces: LLM (AI text), PUBLISHER (OAuth clients), VIDEO_ENGINE
 * (moving-picture providers), IMAGE (image providers).
 * Resolution always prefers the org vault over process env.
...
import { LLM_PROVIDERS, LLM_PROVIDER_MAP, type LlmCredential, type LlmProviderDef } from '../ai/providers.js';
import { VIDEO_PROVIDERS, type VideoCredential, type VideoProviderDef } from '../ai/providers-video.js';

export type VaultCapability = 'LLM' | 'PUBLISHER' | 'VIDEO_ENGINE' | 'IMAGE';
...
export class OrgCredentialsService {
  ...
  private vault(): SecretEnvelope {
    ...
    if (!key) {
      throw new PipelineError('PLATFORM_ERROR', 'Service Unavailable', {
        status: 503,
        detail:
          'Secrets vault key not configured: set SECRETS_MASTER_KEY (64-hex) on the API service — required to store org provider keys encrypted at rest.',
      });
    }
  ...
  ): Promise<Array<{ provider: string; hint: string; validatedAt: string | null }>> {
  ...
  // ── TikTok OAuth (same vault, separate provider key) ─────────────────────
  async resolveTikTokOAuth(orgId: string): Promise<TikTokClientCreds | null> {
    const stored = await this.readSecret(orgId, 'PUBLISHER', TIKTOK_OAUTH_PROVIDER);
    const clientKey = typeof stored?.['clientKey'] === 'string' ? (stored['clientKey'] as string) : '';
    const clientSecret = typeof stored?.['clientSecret'] === 'string' ? (stored['clientSecret'] as string) : '';
    if (stored && clientKey && clientSecret) return { clientKey, clientSecret, source: 'org' };
    const { tiktokClientKey, tiktokClientSecret } = this.config.platforms;
    if (tiktokClientKey && tiktokClientSecret) return { clientKey: tiktokClientKey, clientSecret: tiktokClientSecret, source: 'env' };
    return null;
  }

  async saveTikTokOAuth(orgId: string, clientKey: string, clientSecret: string): Promise<void> {
    await this.writeSecret(orgId, 'PUBLISHER', TIKTOK_OAUTH_PROVIDER, {
      secret: clientSecret,
      clientKey,
      clientSecret,
      hint: OrgCredentialsService.hint(clientKey),
      validatedAt: new Date().toISOString(),
    });
  }
