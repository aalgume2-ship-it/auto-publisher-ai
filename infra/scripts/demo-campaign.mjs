 ...
 * stream check. Works against BOTH the web proxy (http://host:4500, paths
 * under /api/v1) and the API directly (http://host:4000, paths under /v1) —
 * auto-detected via /health/ready.
 *
...
async function resolvePrefix() {
  try {
    ...
    if (r.ok) return '/v1'; // direct API
  } catch { /* ignore */ }
  return '/api/v1'; // web proxy
}
const PREFIX = await resolvePrefix();

async function api(path, init) {
  const res = await fetch(`${BASE}${PREFIX}${path}`, { ...init, headers: { accept: 'application/json', ...(init?.headers ?? {}) } });
...
async function main() {
  log(`▶ demo campaign via ${BASE}${PREFIX}`);
  ...
  // 2) org
  const orgs = await api('/organizations', { headers: auth });
  ...
  const seriesList = await api(`/organizations/${orgId}/series`, { headers: auth });
  const existing = (seriesList.body?.items ?? []).find((s) => s.name === SERIES_NAME);
  let seriesId = existing?.id;
  if (!seriesId) {
    const series = await api(`/organizations/${orgId}/series`, {
      method: 'POST',
      headers: auth,
  ...
  const listResp = await api(`/organizations/${orgId}/videos?seriesId=${seriesId}`, { headers: auth });
  ...
  for (const keyword of KEYWORDS) {
    ...
    const v = await api(`/organizations/${orgId}/series/${seriesId}/videos`, {
      method: 'POST',
      headers: auth,
  ...
  while (pending().size > 0 && Date.now() < deadline) {
    ...
    for (const id of pending()) {
      const r = await api(`/organizations/${orgId}/videos/${id}`, { headers: auth });
  ...
  const finalList = (await api(`/organizations/${orgId}/videos?seriesId=${seriesId}`, { headers: auth })).body?.items ?? [];
  for (const v of finalList.filter((v) => v.seriesId === seriesId)) {
    const res = await fetch(`${BASE}${PREFIX}/organizations/${orgId}/videos/${v.id}/stream`, {
  ...
  log(`★ Open the site and login to view «عطر الصيف»`);
}
